...
**Mitigation:** every binary download verifies a SHA256 pin. The `fqe-v0.18.4` tag in `booyajones/fqe` is the only thing workflows clone, and the workflow uses `git clone --branch fqe-v0.18.4` (not HEAD).
...
**Mitigation:** tags can be force-moved, which is the supply-chain risk here. Defences: (1) signed commits on the public repo, (2) branch protection on `main` requiring review, (3) the workflow can be re-pinned to a SHA instead of a tag for higher assurance (`--branch fqe-v0.18.4` becomes `--branch <40-char SHA>`).
...
3. **Pin `fqe-v0.18.4` to a SHA** in your workflow (look up via `git rev-parse fqe-v0.18.4`).
