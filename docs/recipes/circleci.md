...
- **Pin the ref, not HEAD.** The `FQE_REF` fetch is the supply-chain boundary. For higher assurance set it to a commit SHA, resolved without a local clone via `git ls-remote https://github.com/booyajones/fqe.git 'refs/tags/fqe-v0.18.13^{}' | cut -f1`; the fetch form accepts a tag or a SHA, which `git clone --branch` does not.
