...
cd ~/.local/share/fqe && git checkout fqe-v0.18.15
npm link --prefix ~/.local/share/fqe/cli   # adds `fqe` to PATH

# Option B: one-off via npx (no install, slower)
alias fqe='npx --yes -p github:booyajones/fqe#fqe-v0.18.15 fqe'
```
...
```bash
npx --yes -p github:booyajones/fqe#fqe-v0.18.15 fqe init --with-mutation
