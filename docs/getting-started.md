...
cd ~/.local/share/fqe && git checkout fqe-v0.3.0
npm link --prefix ~/.local/share/fqe/cli   # adds `fqe` to PATH

# Option B: one-off via npx (no install, slower)
alias fqe='npx --yes github:booyajones/fqe#fqe-v0.3.0 cli/bin/fqe.js'
```
...
The default install command in Step 1 uses `fqe-v0.3.0`, a git tag. Tags can be force-pushed, so if upstream's tag is moved (accidentally or maliciously) every repo running the tag-pinned install picks up the new code on the next CI run.
