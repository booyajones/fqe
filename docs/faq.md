...
Tags follow semver: `fqe-v<major>.<minor>.<patch>`. To upgrade, find the new tag in github.com/booyajones/fqe, then update the `FQE_REF="fqe-v0.18.14"` line in `.github/workflows/fqe-quality.yml` (it takes a tag or a 40-char SHA). Breaking changes are called out per release in `CHANGELOG.md`.
