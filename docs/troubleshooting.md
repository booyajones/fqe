...
**Fix:** this was fixed back in v0.1.0, so every current release already carries it. Re-pin the workflow's `FQE_REF` to `fqe-v0.18.21` (the current tag) to pick it up. If you've pinned an older SHA, bump it.
