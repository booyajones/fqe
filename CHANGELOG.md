...
- **Nothing kept the two doc guards in step, and they share two pieces of logic by COPY.** `check_test_count.js` cannot `require` `doc_accuracy.test.js` (a suite cannot spawn itself) and no test reads that script at all, so a change to `isNestedCheckout` or the skip-list in one file and not the other was invisible to all 921 tests. That is not hypothetical: the two lists HAD drifted, `check_test_count.js` having been missing `.fqe-out` and `reports` since v0.18.2, and the nested-worktree fix had to be hand-written into both files across two commits. A test now compares both copies — the function bodies with comments stripped, and the skip-lists as sets — so the next divergence is loud. Found by review, which noted the fix for a sibling-miss was itself unguarded against becoming one.
- The skip-lists are equal again (`.fqe-out` and `reports` added to `check_test_count.js`). Stated honestly: that closes no defect that reproduces today. A generated receipt was checked and carries no `N tests` string, and neither directory exists in a normal tree, so nothing was being mis-scanned. It is parity, and now it is enforced parity.
- **`MIN_COUNT_CLAIMS` sits AT the current count, where its two siblings sit deliberately below theirs,** and the comment now says so instead of citing them as precedent for the opposite. Their reasoning does not carry at this size: they police 26 and 11 claims, where a few claims of margin absorb ordinary editing and still catch a collapse. There are exactly three here, so any margin licenses losing a third of the coverage silently. The cost is that a reword which stops matching the pattern fails CI — loudly, naming the count it found and saying to lower the constant in the same commit, which is a visible decision rather than a hidden one.


- **The nested-checkout rule trusted a path's NAME, so a stray `.git` could silently prune a subtree.** `fs.existsSync(dir/.git)` reads a plain file called `.git`, or an empty directory called `.git`, as a checkout — a misplaced fixture or a debug artifact would have removed that whole subtree from both guards with no error and nothing naming what was skipped. That is the same silent-narrowing this rule was added to close, reintroduced one level down inside the fix for it. Both guards now validate the marker: a worktree's `.git` is a file beginning `gitdir:`, a clone's is a directory containing `HEAD`, and anything else is an ordinary directory and gets scanned. Found by review; nothing in the tree triggers it today.
- Tightening that exposed an unrealistic fixture in the test written for the rule: its "nested clone" was an empty `.git` directory, which a real clone never is. The fixture now builds one with a `HEAD`, and the two shapes that are NOT checkouts are asserted to still be scanned — the rule is pinned from both sides rather than only from the side that skips.
- **`check_test_count.js` had no floor worth the name.** Its only guard was `checked === 0`, so losing two of the three claims it exists to verify still reported success — while `doc_accuracy.test.js` next door carries named `MIN_PIN_CLAIMS`/`MIN_SIZE_CLAIMS` floors with a comment saying a skip-path must not silently become "checked nothing." This file had the weaker half of that idea. `MIN_COUNT_CLAIMS = 3` now makes a DROP loud, not just a wipe-out.
- **Corrected, because the number that motivated a second look was wrong:** review reported guard coverage falling from 60 pin claims to 26, reading as a floor margin far thinner than assumed. Measured on a clean clone, which is what CI and every adopter sees, coverage is **26 before and 26 after** — identical, and the whole change is a no-op there. The 52-vs-26 figure exists only in a working tree that has a nested worktree, where every file was being counted twice. `MIN_PIN_CLAIMS = 20` has always had a margin of 6, consistent with the note beside it recording a real drop of 25 -> 19. No floor was re-calibrated on the strength of an inflated baseline.
...
- **A pull request based on anything other than `main` ran neither the suite matrix nor the doc test-count job, and still showed a green checks list.** `tests.yml` filtered its `pull_request` trigger to `branches: [main]`, so stacked work — the normal shape for a fix built on an open PR, and how this very change reached review — was gated only by the workflows that do fire on any base. The other checks passing made that look like coverage. The header comment on that file has always claimed "every push and PR", and `README.md` says the same; the trigger did not match either. This is the same defect shape as the parser fixes above: a guard whose stated scope and actual scope differ, reporting success over a range it never evaluated — which is the failure `[0.18.20]` spent ten rounds on for `fqe run`'s own diff scope. The filter is gone. `push` stays pinned to `main`, since a branch's pushes are covered by its PR.
