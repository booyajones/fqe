...
- **A block-scalar introducer (`key: |`) parsed to the one-character string `"|"`.** It is now rejected where the author wrote it, and a block sequence under a runner field is named as such rather than reported only as a bad indent, since that is the shape `docs/writing-a-runner.md` shipped until v0.18.20.
